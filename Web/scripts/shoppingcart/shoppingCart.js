    ...
    if (items != null && JSON != null) {
        try {
            ...
            for (var i = 0; i < items.length; i++) {
                var item = items[i];
                if (item.sku != null && item.name != null && item.price != null && item.quantity != null && item.fileUrl!=null) {
                    item = new cartItem(item.sku, item.name, item.price, item.quantity,fileUrl,category,description);
...
shoppingCart.prototype.addItem = function (sku, name, price, quantity,fileUrl,category,description) {
    ...
    if (quantity != 0) {
        ...
        // new item, add now
        if (!found) {
            var item = new cartItem(sku, name, price, quantity, fileUrl, category, description);
            this.items.push(item);
        }
...
// items in the cart
//
function cartItem(sku, name, price, quantity,fileUrl,category,description) {
    this.sku = sku;
    this.name = name;
    this.price = price * 1;
